...
class NeuralNetwork:
    ...
    def momentum(self, layers, layers_deltas, alpha, beta=0.9):
        for layer_index in reversed(xrange(len(layers) - 1)):
            gradient = layers[layer_index].T.dot(layers_deltas[layer_index])
    ...
    def testgd(self, layers, layers_deltas, alpha):
        if not hasattr(self, "test_vel"):
            self.test_vel = [numpy.zeros_like(weights) for weights in self.weights_list]
        if not hasattr(self, "test_acc"):
            self.test_acc = [numpy.zeros_like(weights) for weights in self.weights_list]

        for layer_index in reversed(xrange(len(layers) - 1)):
            gradient = layers[layer_index].T.dot(layers_deltas[layer_index])
            self.weights_list[layer_index] += alpha * gradient

    def train(self, training_inputs, training_outputs, iterations=1000, alpha=1.0, beta=0.9,
